...
from networks.TAunet import TAUNet
...
def test_ce_net_vessel():
    source = './dataset/CHASEDB/test/images/'
    val = os.listdir(source)
    disc = 50
    solver = TTAFrame(TAUNet)
    # solver.load('weights/log01_dink34-DCENET-DRIVE.th')
    solver.load('/home/changwei/cenet/weights/TAUnet_CHASEDB.th')
    tic = time()
    target = './submits/log_CE_Net/'
    if not os.path.exists(target):
        os.mkdir(target)
    gt_root = './dataset/CHASEDB/test/1st_manual'
    total_m1 = 0
    ...
    for i, name in enumerate(val):
        ...
        ground_truth_path = os.path.join(gt_root, name.split('.')[0] + '_1stHO.png')
